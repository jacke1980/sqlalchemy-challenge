...
def welcome():
    return (
        ...
        f"<p>Usage:</p>"
        f"/api/v1.0/precipitation<br/>Returns a JSON list of percipitation data for dates between 8/23/16 and 8/23/17<br/><br/>"
...
def precipitation():
    ...
    # Date 12 months ago
    results_pr = session.query(Measurement.date, func.avg(Measurement.prcp)).filter(Measurement.date >= last_twelve_months).group_by(Measurement.date).all()
    return jsonify(results_pr)
...
def stations():
    results_st = session.query(Station.station, Station.name).all()
    return jsonify(results_st)
